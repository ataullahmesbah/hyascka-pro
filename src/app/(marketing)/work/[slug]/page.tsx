        ...
        <div className="container">
          <dl className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {study.metrics.map((metric) => (
              <div key={metric.label} className="brand-ring rounded-xl border border-border bg-card p-6">
                <dt className="text-xs uppercase tracking-wider text-muted-foreground">{metric.label}</dt>
                <dd className="brand-text mt-2 font-display text-2xl font-extrabold">{metric.value}</dd>
              </div>
            ))}
          </dl>

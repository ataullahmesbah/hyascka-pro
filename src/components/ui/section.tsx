...
export function SectionHeading({
  ...
  return (
    <Reveal className={cn("max-w-[60ch]", align === "center" && "mx-auto text-center", className)}>
      ...
      <h2 className={cn("text-step-4 font-bold tracking-tight", eyebrow && "mt-4")}>{title}</h2>
      {description ? (
  ...
  return (
    <header className="hero-surface relative overflow-hidden border-b border-line">
      ...
      <div className="container-x relative py-14 md:py-20">
        {eyebrow ? <span className="eyebrow">{eyebrow}</span> : null}
        <h1 className={cn("max-w-[22ch] text-step-5 font-bold tracking-tight", eyebrow && "mt-5")}>{title}</h1>
        {description ? (
